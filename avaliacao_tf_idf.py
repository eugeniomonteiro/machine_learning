# -*- coding: utf-8 -*-
"""
Created on Sat Sep  5 08:47:55 2020
...
#*****************************************************************************
#
#        As acurácias ficam na variável df_resultados
#
#*****************************************************************************
#*****************************************************************************
#
#       As saídas de cada modelo ficam em df_saidas_modelos
#
#*****************************************************************************
...
def criar_corpus(texto):
    #Aqui não precisa remover stopword pois o TdidfVectorizer já faz isso
...
#==============================================================================
#DATA FRAMES QUE ARMAZENAM OS RESULTADOS
...
for i in range(0,10):
   ...
   corpus_atyp = criar_corpus(x_atyp)
   vectorizer = TfidfVectorizer(stop_words = 'english', max_df = 0.95, min_df = 0.05)
   vec = vectorizer.fit_transform(corpus_atyp)
   tfidf_atyp = vec.todense()
   
   corpus_beta = criar_corpus(x_beta)
   vectorizer = TfidfVectorizer(stop_words = 'english', max_df = 0.95, min_df = 0.05)
   vec = vectorizer.fit_transform(corpus_beta)
   tfidf_beta = vec.todense()
   
   corpus_calcium = criar_corpus(x_calcium)
   vectorizer = TfidfVectorizer(stop_words = 'english', max_df = 0.95, min_df = 0.05)
   vec = vectorizer.fit_transform(corpus_calcium)
   tfidf_calcium = vec.todense()   
   
   corpus_proton = criar_corpus(x_proton)
   vectorizer = TfidfVectorizer(stop_words = 'english', max_df = 0.95, min_df = 0.05)
   vec = vectorizer.fit_transform(corpus_proton)
   tfidf_proton = vec.todense()
   ...
   x_trein_atyp, x_test_atyp, y_trein_atyp, y_test_atyp = train_test_split(tfidf_atyp, y_atyp, test_size=0.20, random_state=0)
   # y_trein_atyp = y_trein_atyp.values
   # y_test_atyp = y_test_atyp.values

   x_trein_beta, x_test_beta, y_trein_beta, y_test_beta = train_test_split(tfidf_beta, y_beta, test_size=0.20, random_state=0)
   # y_trein_beta = y_trein_beta.values
   # y_test_beta = y_test_beta.values

   x_trein_calcium, x_test_calcium, y_trein_calcium, y_test_calcium = train_test_split(tfidf_calcium, y_calcium, test_size=0.20, random_state=0)
   # y_trein_calcium = y_trein_calcium.values
   # y_test_calcium = y_test_calcium.values

   x_trein_proton, x_test_proton, y_trein_proton, y_test_proton = train_test_split(tfidf_proton, y_proton, test_size=0.20, random_state=0)
...
df_saidas_reais.to_excel('saidas_reais_tfidf_novo.xls')
df_resultados.to_excel('resultados_tfidf_novo.xls')
df_saidas_modelos.to_excel('saidas_modelos_tfidf_novo.xls')